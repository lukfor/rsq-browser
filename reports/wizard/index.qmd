...
#write.table(col_names, "colnames.txt", row.names = F, col.names = F)

## faster -> read colnames from fixed colnames.txt file - TODO: create .txt files for all population files
#col_names <- as.character(unlist(fread("colnames.txt", header = F)))
...
for (i in 1:length(params$genes_coordinates)) {
  ...
  styled_table <- list(datatable(gene_stats_temp, caption = "") %>%
