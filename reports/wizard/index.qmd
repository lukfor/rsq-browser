...
get_gene_statistics <- function(input_df, gene_name) {
  ...
  ### collect occuring bins in vector (CAVE: some genes might for instance not have any ultra-rare variants, code needs to account for this)
  occuring_bins <- unique(as.character(input_df$bin))
  ...
  for (i in 1:length(array_ref_ids)) {
    ...
    perc_per_bin_subset <- count_per_bin_subset %>%
      ## divide all occuring(!) MAF bin columns by nr_var
      mutate(across(c(occuring_bins), ~ . /nr_var))
