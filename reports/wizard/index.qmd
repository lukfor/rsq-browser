...
if (params$genotyped == TRUE) {
  ...
  ### get actual full chip name for params$chip using vector created above (bc. for gene/SNP/PGS stats renaming of chips occurs BEFORE filtering)
...
#overview_figure_header <- list(tags$h3("Genome-wide Imputation Quality"))
  
## append to the html_list (list of lists -> every sublist is markdown code, which should be displayed)
#html_list_overview <- append(html_list_overview, overview_figure_header)
...
## add to html list
#html_list_overview <- append(html_list_overview, list(lineplot))
...
#### collect scoring data in separate df
...
if (length(params$genes_coordinates) != 0) {
  ...
  for (i in 1:length(params$genes_coordinates)) {
    ...
    # gene_stats_temp$score <- scale_to_range(gene_stats_temp$ultra_rare.mean_r2, 0, 2) +
    #                  scale_to_range(gene_stats_temp$rare.mean_r2, 0, 1.5) +
    #                  scale_to_range(gene_stats_temp$low_freq.mean_r2, 0, 1) +
    #                  scale_to_range(gene_stats_temp$common.mean_r2, 0, 1) +
    #                  scale_to_range(gene_stats_temp$nr_variants, 0, 1)
...
ranking_table <- list(datatable(ranking, filter = "top", rownames = FALSE, 
                                class = 'display',  # Add a class to target CSS
                                colnames = c("Genotyping Array" = "genotyping_array", ### adjust colnames
                                                  "Reference Panel" = "ref_panel",
                                                  "Total Performance Score" = "total_score"), 
                                options = list(pageLength = nrow(pgs_queried), ## show all rows
                                    lengthChange = F, ## remove "Show entries" dropdown
                                    paging = F ## disable pagination of Table (not required when all rows are shown)
                                    #searching = FALSE ## remove search bar, BUT then the filter no longer works!
                                    )) %>%
                                    formatRound(c('Total Performance Score'), digits = 3))


header_ranking <- list(tags$h3("Performance Evaluation"))

html_list_overview <- append(html_list_overview, header_ranking)


html_list_overview <- append(html_list_overview, ranking_table)


barplot1 <- ggplot(ranking, aes(x = genotyping_array, y = total_score, fill = ref_panel)) +
  geom_bar(stat = "identity", position = "dodge") +
  #labs(title = "Barplot Grouped by Genotyping Array", x = "Genotyping Array", y = "Total Score") +
  ggtitle("Barplot Grouped by Genotyping Array")+
...
#barplot1_plotly <- ggplotly(barplot1) %>% layout(legendgroup = "group1", showlegend = T, legend = list(x = 0.3, y = 0.5))

#html_list <- append(html_list, list(barplot1_plotly))

barplot2 <- ggplot(ranking, aes(x = ref_panel, y = total_score, fill = genotyping_array)) +
  geom_bar(stat = "identity", position = "dodge") +
  #labs(title = "Barplot Grouped by Reference Panel", x = "Reference Panel", y = "Total Score") +
  ggtitle("Barplot Grouped by Reference Panel")+
...
#barplot2_plotly <- ggplotly(barplot2) %>% layout(legendgroup = "group2", showlegend = T, legend = list(x = 0.95, y = 0.5))


#combined <- subplot(barplot1_plotly, barplot2_plotly, nrows = 1)
  # layout(
  #   annotations = list(
  #     list(x = 0.25, y = 1.1, text = "Plot 1 Title", showarrow = FALSE, xref = "paper", yref = "paper"),
  #     list(x = 0.75, y = 1.1, text = "Plot 2 Title", showarrow = FALSE, xref = "paper", yref = "paper")
  #   ),
  #   xaxis = list(title = "X-axis Title 1"),
  #   xaxis2 = list(title = "X-axis Title 2"),
  #   yaxis = list(title = "Y-axis Title 1"),
  #   yaxis2 = list(title = "Y-axis Title 2")
  # )

#html_list <- append(html_list, list(barplot2_plotly))

#html_list <- append(html_list, list(combined))
...
```{r pop_section, echo=FALSE, warning=FALSE, message=FALSE, results='asis'}
