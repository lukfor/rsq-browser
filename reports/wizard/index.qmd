...
get_gene_statistics <- function(input_df, gene_name) {
  ...
  ## get mean r2 for each bin and for each array/ref-panel combination
  ...
  for (i in 1:length(array_ref_ids)) {
    ...
    stats_temp$mean_r2_ultra_rare <- ref_data_bins_subset$ultra_rare
    stats_temp$mean_r2_rare <- ref_data_bins_subset$rare
    stats_temp$mean_r2_low_freq <- ref_data_bins_subset$low_frequency
    stats_temp$mean_r2_common <- ref_data_bins_subset$common
...
get_snp_statistics <- function(input_df, snp_name) {
  ...
  for (i in 1:length(array_ref_ids)) {
    ...
    stats_temp$genotyping_array <- genotyping_array
    stats_temp$ref_panel <-ref_panel_name
    stats_temp$included <- included
    stats_temp$r2 <- r2
...
for (i in 1:length(params$genes_coordinates)) {
  ...
  styled_table <- list(datatable(gene_stats_temp, options = list(
    ...
    formatStyle(c('mean_r2_ultra_rare', 'mean_r2_rare', 'mean_r2_low_freq', 'mean_r2_common'),
...
if (length(params$snps) != 0) {
  ...
  styled_snp_table <- list(datatable(snp_data,
                                     extensions = 'RowGroup', ### use rowgroup extension
                                     options = list(rowGroup = list(dataSrc = 1), ### group rows by SNP (first column)
                                                    columnDefs = list(list(visible = F, targets = 1)))) %>% ### hide first column from display
      ### set custom format for selected columns
      formatStyle(c('r2'),
                backgroundColor = styleInterval(c(0.6, 0.8), c('darkred', 'orange', 'darkgreen')),
                color = c("white")))
